Gem::Specification.new do |s|
  s.name        = "figurate_numbers"
  s.version     = "0.5.0"
  s.summary     = "Generates infinite sequences of plane, space and multidimensional figurate numbers based on the book Figurate Numbers(2012) by Elena Deza and Michel Deza."
  s.description = "Infinite sequences of plane, space and multidimensional figurate numbers using the Enumerator class. Based on the book Figurate Numbers(2012) by Elena Deza & Michel Deza, it is intended for use in Sonic Pi."
  s.authors     = ["Edgar Armando Delgado Vega"]
  s.email       = "edelve91@gmail.com"
  s.files       = ["lib/figurate_numbers.rb"]
  s.homepage    =
    "https://github.com/edelveart/figurate_numbers_sonic_pi"
  s.license       = "MIT"
end
