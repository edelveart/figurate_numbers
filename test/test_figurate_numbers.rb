...
class TestFigurateNumbers  < Minitest::Test
  ...
  def test_polite_numbers
    assert_equal([1, 3, 5, 6, 7, 9, 10, 11, 12, 13, 14, 15, 17, 18, 19], FigurateNumbers.polite_numbers.take(15).to_a)
  end

  def test_impolite_numbers
    assert_equal([1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384], FigurateNumbers.impolite_numbers.take(15).to_a)
  end
  ...
  def test_triangular_pyramidal_numbers
    assert_equal([1, 4, 10, 20, 35, 56, 84, 120, 165, 220, 286, 364, 455, 560, 680, 816, 969, 1140, 1330, 1540], FigurateNumbers.triangular_pyramidal_numbers.take(20))
  end

  def test_square_pyramidal_numbers
    assert_equal([1, 5, 14, 30, 55, 91, 140, 204, 285, 385, 506, 650, 819, 1015, 1240, 1496, 1785, 2109, 2470, 2870], FigurateNumbers.square_pyramidal_numbers.take(20))
  end

  def test_pentagonal_pyramidal_numbers
    assert_equal([1, 6, 18, 40, 75, 126, 196, 288, 405, 550, 726, 936, 1183, 1470, 1800, 2176, 2601, 3078, 3610, 4200], FigurateNumbers.pentagonal_pyramidal_numbers.take(20))
  end

  def test_hexagonal_pyramidal_numbers
    assert_equal([1, 7, 22, 50, 95, 161, 252, 372, 525, 715, 946, 1222, 1547, 1925, 2360, 2856, 3417, 4047, 4750,
    5530], FigurateNumbers.hexagonal_pyramidal_numbers.take(20))
  end

  def test_heptagonal_pyramidal_numbers
    assert_equal([1, 8, 26, 60, 115, 196, 308, 456, 645, 880, 1166, 1508, 1911, 2380, 2920, 3536, 4233, 5016, 5890, 6860], FigurateNumbers.heptagonal_pyramidal_numbers.take(20))
  end

  def test_octagonal_pyramidal_numbers
    assert_equal([1, 9, 30, 70, 135, 231, 364, 540, 765, 1045, 1386, 1794, 2275, 2835, 3480, 4216, 5049, 5985, 7030, 8190], FigurateNumbers.octagonal_pyramidal_numbers.take(20))
  end

  def test_nonagonal_pyramidal_numbers
    assert_equal([1, 10, 34, 80, 155, 266, 420, 624, 885, 1210, 1606, 2080, 2639, 3290, 4040, 4896, 5865, 6954, 8170, 9520], FigurateNumbers.nonagonal_pyramidal_numbers.take(20))
  end

  def test_decagonal_pyramidal_numbers
    assert_equal([1, 11, 38, 90, 175, 301, 476, 708, 1005, 1375, 1826, 2366, 3003, 3745, 4600, 5576, 6681, 7923, 9310, 10850], FigurateNumbers.decagonal_pyramidal_numbers.take(20))
  end

  def test_hendecagonal_pyramidal_numbers
    assert_equal([1, 12, 42, 100, 195, 336, 532, 792, 1125, 1540, 2046, 2652, 3367, 4200, 5160, 6256, 7497, 8892,
    10450, 12180], FigurateNumbers.hendecagonal_pyramidal_numbers.take(20))
  end

  def test_dodecagonal_pyramidal_numbers
    assert_equal([1, 13, 46, 110, 215, 371, 588, 876, 1245, 1705, 2266, 2938, 3731, 4655, 5720, 6936, 8313, 9861,
    11590, 13510], FigurateNumbers.dodecagonal_pyramidal_numbers.take(20))
  end

  def test_tridecagonal_pyramidal_numbers
    assert_equal([1, 14, 50, 120, 235, 406, 644, 960, 1365, 1870, 2486, 3224, 4095, 5110, 6280, 7616, 9129, 10830, 12730, 14840], FigurateNumbers.tridecagonal_pyramidal_numbers.take(20))
  end

  def test_tetradecagonal_pyramidal_numbers
    assert_equal([1, 15, 54, 130, 255, 441, 700, 1044, 1485, 2035, 2706, 3510, 4459, 5565, 6840, 8296, 9945, 11799, 13870, 16170], FigurateNumbers.tetradecagonal_pyramidal_numbers.take(20))
  end

  def test_pentadecagonal_pyramidal_numbers
  ...
  def test_icosihenagonal_pyramidal_numbers
    assert_equal([1, 22, 82, 200, 395, 686, 1092, 1632, 2325, 3190, 4246, 5512, 7007, 8750, 10760, 13056, 15657, 18582, 21850, 25480], FigurateNumbers.icosihenagonal_pyramidal_numbers.take(20))
  end

  def test_icosidigonal_pyramidal_numbers
    assert_equal([1, 23, 86, 210, 415, 721, 1148, 1716, 2445, 3355, 4466, 5798, 7371, 9205, 11320, 13736, 16473, 19551, 22990, 26810], FigurateNumbers.icosidigonal_pyramidal_numbers.take(20))
  end

  def test_icositrigonal_pyramidal_numbers
    assert_equal([1, 24, 90, 220, 435, 756, 1204, 1800, 2565, 3520, 4686, 6084, 7735, 9660, 11880, 14416, 17289, 20520, 24130, 28140], FigurateNumbers.icositrigonal_pyramidal_numbers.take(20))
  end


  def test_icositetragonal_pyramidal_numbers
    assert_equal([1, 25, 94, 230, 455, 791, 1260, 1884, 2685, 3685, 4906, 6370, 8099, 10115, 12440, 15096, 18105,
    21489, 25270, 29470], FigurateNumbers.icositetragonal_pyramidal_numbers.take(20))
  end

  def test_icosipentagonal_pyramidal_numbers
    assert_equal([1, 26, 98, 240, 475, 826, 1316, 1968, 2805, 3850, 5126, 6656, 8463, 10570, 13000, 15776, 18921,
    22458, 26410, 30800], FigurateNumbers.icosipentagonal_pyramidal_numbers.take(20))
  end

  def test_icosihexagonal_pyramidal_numbers
    assert_equal([1, 27, 102, 250, 495, 861, 1372, 2052, 2925, 4015, 5346, 6942, 8827, 11025, 13560, 16456, 19737, 23427, 27550, 32130], FigurateNumbers.icosihexagonal_pyramidal_numbers.take(20))
  end

  def test_icosiheptagonal_pyramidal_numbers
    assert_equal([1, 28, 106, 260, 515, 896, 1428, 2136, 3045, 4180, 5566, 7228, 9191, 11480, 14120, 17136, 20553, 24396, 28690, 33460], FigurateNumbers.icosiheptagonal_pyramidal_numbers.take(20))
  end

  def test_icosioctagonal_pyramidal_numbers
    assert_equal([1, 29, 110, 270, 535, 931, 1484, 2220, 3165, 4345, 5786, 7514, 9555, 11935, 14680, 17816, 21369, 25365, 29830, 34790], FigurateNumbers.icosioctagonal_pyramidal_numbers.take(20))
  end

  def test_icosinonagonal_pyramidal_numbers
    assert_equal([1, 30, 114, 280, 555, 966, 1540, 2304, 3285, 4510, 6006, 7800, 9919, 12390, 15240, 18496, 22185, 26334, 30970, 36120], FigurateNumbers.icosinonagonal_pyramidal_numbers.take(20))
  end

  def test_triacontagonal_pyramidal_numbers
    assert_equal([1, 31, 118, 290, 575, 1001, 1596, 2388, 3405, 4675, 6226, 8086, 10283, 12845, 15800, 19176, 23001, 27303, 32110, 37450], FigurateNumbers.triacontagonal_pyramidal_numbers.take(20))
  end
  ...
  def test_five_dimensional_square_pyramidal_numbers
    assert_equal([1, 7, 27, 77, 182, 378, 714, 1254, 2079, 3289], FigurateNumbers.five_dimensional_square_pyramidal_numbers.take(10))
  end

  def test_five_dimensional_pentagonal_pyramidal_numbers
    assert_equal([1, 8, 33, 98, 238, 504, 966, 1716, 2871, 4576], FigurateNumbers.five_dimensional_pentagonal_pyramidal_numbers.take(10))
  end

  def test_five_dimensional_hexagonal_pyramidal_numbers
    assert_equal([1, 9, 39, 119, 294, 630, 1218, 2178, 3663, 5863], FigurateNumbers.five_dimensional_hexagonal_pyramidal_numbers.take(10))
  end

  def test_five_dimensional_heptagonal_pyramidal_numbers
    assert_equal([1, 10, 45, 140, 350, 756, 1470, 2640, 4455, 7150], FigurateNumbers.five_dimensional_heptagonal_pyramidal_numbers.take(10))
  end

  def test_five_dimensional_octagonal_pyramidal_numbers
    assert_equal([1, 11, 51, 161, 406, 882, 1722, 3102, 5247, 8437], FigurateNumbers.five_dimensional_octagonal_pyramidal_numbers.take(10))
  end
  ...
  def test_generalized_biquadratic_numbers
    assert_equal([0, 1, 16, 81, 256, 625, 1296, 2401, 4096, 6561, 10000, 14641, 20736, 28561, 38416, 50625, 65536, 83521, 104976, 130321], FigurateNumbers.generalized_biquadratic_numbers.take(20))
  end
  ...
  def test_generalized_hyperoctahedral_numbers
    assert_equal([0, 1, 8, 33, 96, 225, 456, 833, 1408, 2241, 3400, 4961, 7008, 9633, 12936, 17025, 22016, 28033, 35208, 43681], FigurateNumbers.generalized_hyperoctahedral_numbers.take(20))
  end
  ...
  def test_generalized_k_dimensional_centered_hypertetrahedron_numbers
    assert_equal([-11404, -6427, -3431, -1716, -792, -330, -120, -36, -8, -1, 0, 1, 8, 36, 120, 330, 792, 1716, 3431, 6427], FigurateNumbers.generalized_k_dimensional_centered_hypertetrahedron_numbers(6, 10).take(20))
  end

  def test_generalized_k_dimensional_centered_hyperoctahedron_numbers
    assert_equal([-5984767, -3800305, -2340495, -1392065, -795455, -433905, -224143, -108545, -48639, -19825, -7183, -2241, -575, -113, -15, 1, 15, 113, 575, 2241, 7183, 19825, 48639, 108545, 224143], FigurateNumbers.generalized_k_dimensional_centered_hyperoctahedron_numbers(7, 15).take(25))
  end
